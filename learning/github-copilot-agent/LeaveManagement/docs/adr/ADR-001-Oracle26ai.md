...
Application code will access the database using Dapper. Database scripts, schema files, and seed data will be stored under `database/` when those assets are created.
...
This decision also supports the project learning goals, which include connecting .NET backend application code to Oracle 26ai and organizing database assets in a version-controlled repository.
...
- Application data access code should use Dapper with parameterized SQL.
