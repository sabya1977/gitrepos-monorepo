...
'<DECISAO>
  ...
  <TX_EMENTA>
  <tp>RECURSO ORDINÁRIO. INVESTIGAÇÃO JUDICIAL. ABUSO DE PODER. PRELIMINAR. JOAQUIM DOMINGOS RORIZ PREJUDICIALIDADE. JULGAMENTO RCED Nº 613 e RO Nº 738. SANÇÃO PREVISTA NO ART. 1º, I, d, da LC Nº 64/90. INOCUIDADE. PREJUDICIALIDADE QUANTO
...
begin
  ...
  ctx_ddl.add_zone_section ('xmlsjurgroup', 'tp', 'tp');
...
begin
  ctx_ddl.drop_preference('smjlex');
exception when others then null;
end;
/

begin
  ctx_ddl.create_preference('smjlex','BASIC_LEXER');
  ctx_ddl.set_attribute('smjlex','base_letter','YES');
END;
/
...
-- this should work
select count(*) from decisao_arquivo where contains ( bi_arquivo_xml, '(joaquim and roriz) within emen'	) > 0;

-- this should return 0
select count(*) from decisao_arquivo where contains ( bi_arquivo_xml, '(joaquim and roriz) within tp'	) > 0;
select count(*) from decisao_arquivo where contains ( bi_arquivo_xml, '(joaquim and anos) within tp'  ) > 0;
