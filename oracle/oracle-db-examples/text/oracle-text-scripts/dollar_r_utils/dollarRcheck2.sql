...
begin
  ...
  loop
    ...
    while (loboff < loblen) loop
      ...
      while (bufoff < (buflen*2)) loop
        ...
        select dump(rid, 16) into drid from dual;
	
        -- ignore nulled-out rowids
	if substr(rid,1,16) != 'AAAAAAAAAAAAAAAA' then
           -- dbms_output.put_line('checking rowid: '||rid || ' ' || drid);
           rowcnt := rowcnt + 1;
	   begin
              select count(*) into rcount from &base_table where rowid = rid;
	   exception when inv_rowid then
	      select dump(rid,16) into dump_rid from dual;
              dbms_output.put_line ('Invalid ROWID for DOCID '||docid||'. ROWID is '||dump_rid);
              raise_application_error(-20000, 'Invalid ROWID for DOCID '||docid||'. ROWID is '||dump_rid);
           end;
           if rcount = 0 then
              dbms_output.put_line ('No such rowid '|| rid || ' for DOCID '||docid);
              raise_application_error(-20000, 'No such rowid '|| rid || ' for DOCID '||docid);
           end if;
        else
	   zero_cnt := zero_cnt + 1;
  ...
  dbms_output.put_line('Checked '||rowcnt||' ROWIDs ('||zero_cnt||' deleted DOCIDs), all are valid.');
end;
/
