...
begin
  ...
  /* first 'ec' in here supports leading wildcards */
  pattern := '(' || lc || ec || '[' || jc1 || '[:alnum:]' || joinC || jc2 || jc3 || ']+' || ec ||
              ')|(' || lc || '".+")';
  ...
  loop
    ...
    dbms_output.put_line('match is <'|| match || '>');

    -- This section is reworked to allow for leading wildcards
    ...
    if substr(match, length(match), 1) = userWild or substr(match, 1, 1) = userWild then
       ...
       if substr(match, length(match), 1) = userWild then
          match := substr(match, 1, length(match)-1) || sysWild;
       end if;

       if substr(match, 1, 1) = userWild then
          match := sysWild || substr(match, 2, length(match)-1);
       end if;
