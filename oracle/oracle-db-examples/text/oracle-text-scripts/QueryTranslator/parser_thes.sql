...
--     THS(term) : THS is a thesaurus operator such as SYN, BT, etc.
...
procedure SetThesOption    (thesOption  integer  );
...
global_thesOption boolean      default FALSE;
...
begin
  ...
  dbms_output.put_line('Joinchars are "' || joinChars || '"');
...
begin
  ...
  -- If thesOption is on, then we add parentheses to the join character list
  -- this is a bit of a hack, as it may break certain queries
  if global_thesOption then
    reParse ( queryString, words, global_joinChars || ')(' );
  else
    reParse ( queryString, words, global_joinChars );
  end if;
...
procedure setThesOption (thesOption integer)
 is
begin
  if thesOption = optionTrue then
    global_thesOption := TRUE;
  elsif thesOption = optionFalse then
    global_thesOption := FALSE;
  else
    raise_application_error (-20706, 'Invalid option for failing on minus only: use 0 for no, 1 for yes');
  end if;
end setThesOption;
