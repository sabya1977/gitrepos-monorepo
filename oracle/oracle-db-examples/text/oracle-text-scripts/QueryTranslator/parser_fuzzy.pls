...
--
-- this version adds support for fuzzy match operator ?
-- use of ? at the start of the term is passed through to the final query.
-- for example "quick ?broon fox"

-- this version also adds synonym support
-- use parser.setThesName('nameofthesaurus')
-- then all terms will be looked up in the thesaurus and any SYNonyms will be expanded.
-- synonym expansion will NOT happen if fuzzy or wildcard operators are used

-- Note: For this version with synonym support, the user running this file must have
-- explicit execute grant on ctx_thes. For example:
-- grant execute on ctxsys.ctx_thes to myusername; 
-- otherwise the procedure will not compile
-- There is no obvious solution to this problem since 
...
--   setThesName      (thesName    varchar2 )
--     Specifies a thesaurus to use. If null, no thesaurus is used.
--     If called, each base term in looked up to find any synonyms, and the base term
--     is expanded to include synonyms
...
-- ORA-20708: Thesaurus name does not exist
...
-- Version history:
--   Version 0.99f:2022-03-29  Experimental support for FUZZY and SYNONYM expansions
...
function  getSynonyms      ( startStr   varchar2,  term varchar2) return varchar2;

procedure setThesName      (thesName    varchar2 );
...
global_thesName  varchar2(128)  default '';
...
is
  ...
  jc2       varchar2(128) := '';
  jc3       varchar2(128) := '';
  fuzzychar varchar2(1)   := '?';
  startStr  varchar2(128);
  term      varchar2(128);
begin
  ...
  pattern := '(' || lc || '[' || jc1 || '[:alnum:]' || joinC || jc2 || jc3 || fuzzychar || ']+' || ec ||
              ')|(' || lc || '"[^"]+")';
  --p(pattern);
  ...
  loop
    ...
    -- OR surround word with braces (braces don't work with wild cards or fuzzy match operator
    -- backslashes don't escape keywords)

    --p('lc is '||lc);
    --p('match is '||match);
    --p('regexp_substr is '|| '^('||lc||'\'||fuzzychar||')'); 
    ...
    if substr(match, length(match), 1) = userWild then
       ...
       match := match || sysWild;
    elsif length (regexp_substr(match, '^('||lc||'\'||fuzzychar||'.*)')) > 0 then 
       --p('match contains a fuzzy operator');
       match := regexp_replace (match, '^('||lc||'\'||fuzzychar||')(.*)', '\1{\2}');
    else
       --p('no fuzzy');
       --p('looking for: '||'^('||lc||')(.*)');
       startStr := regexp_replace(match, '^('||lc||')(.*)', '\1' );
       term     := regexp_replace(match, '^('||lc||')(.*)', '\2' );
       --p('startStr is '||startStr);
       --p('term     is '||term);
       -- match := regexp_replace (match, '^('||lc||')(.*)', '\1{\2}');
       match := getSynonyms(startStr, term);
    end if;
...
function getSynonyms(startStr varchar2, term varchar2) return varchar2 is
  tbuf varchar2(4000);
begin
  if global_thesName is null then
    return startStr || '{' || term || '}';
  else
    tbuf := ctx_thes.syn(term, global_thesName);
    -- ctx_thes.syn expands to an OR list. But OR list is not allowed in phrase
    -- so we need to change OR (|) into EQUIV (|)
    tbuf := regexp_replace(tbuf, '\|', '=');
    return startStr || tbuf;
  end if;
end getSynonyms;
...
procedure setThesName  (thesName varchar2) is
  v_thsName varchar2(128);
begin
  if thesName is null then
    global_thesName := '';
  else
    select ths_name into v_thsName from ctx_user_thesauri
      where upper(thesName) = ths_name;
    global_thesName := v_thsName;
  end if;
 exception 
  when no_data_found then
     raise_application_error (-20708, 'Thesaurus does not exist: '|| thesName);
end setThesName;
