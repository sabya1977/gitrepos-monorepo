...
insert into library_stock values (XMLType('
  <book>
...
select extractValue(book_info, '/book/title') as "Title" from library_stock 
...
declare
  v_rowid rowid;
begin
  select rowid into v_rowid from library_stock 
    where extractValue(book_info, '/book/title') = 'A Prayer for Owen Meany';
