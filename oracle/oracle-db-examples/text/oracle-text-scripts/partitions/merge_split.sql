...
-- now for the split. First we create two temporary tables
...
-- must take that the restriction here match the split condition

INSERT INTO bike_items_temp_1 
  SELECT * FROM bike_items WHERE price BETWEEN 10 AND 50;

INSERT INTO bike_items_temp_2 
  SELECT * FROM bike_items WHERE price BETWEEN 50 AND 100;

-- and create an indexes on both temp tables
-- remember the original table and index is fully usable at this time
-- but we do have to take care of inserts and updates that happen after the copy
