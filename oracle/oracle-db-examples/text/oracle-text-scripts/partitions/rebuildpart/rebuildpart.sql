...
-- make the first partition sync(on commit)
...
-- make the second partition sync(manual)

alter index bike_items_idx rebuild partition p2 parameters ('replace nopopulate sync(manual)');
...
-- make the third partition sync(on commit)

alter index bike_items_idx rebuild partition p3 parameters ('replace populate sync(on commit)');

select id, price, descrip from bike_items where contains (descrip, 'wheel') > 0 order by price;

-- now sync any missing rows

exec ctx_ddl.sync_index('bike_items_idx', part_name => 'p2')
