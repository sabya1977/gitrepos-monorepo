...
int oci_setup() {
  ...
  rc = OCILogon(p_env, p_err, &p_svc, "roger", 5, "roger", 5, "db", 4);
  if (rc != 0) {
...
int main( int argc, char ** argv ) {
  ...
  sprintf (sql, "drop table %s", tab_name );
  execute_sql(sql);

  strcpy( sql, "create table ");
  strcat( sql, tab_name );
  strcat( sql, " (id number primary key, text clob)\n" );

  if ( num_partitions > 1 ) {
    strcat( sql, "partition by range (id) (" );
    range_step = num_docs / num_partitions;
    range_limit = range_step;

    int done = 0;
    while (!done) {
      sprintf( buff, "   %c partition p%d values less than (%d)\n", comma, p_num++, range_limit + 1 );
      strcat( sql, buff );
      comma = ',';
      range_limit += range_step;
      if (range_limit > num_docs) done = 1;
    }
    strcat( sql, ")" );
  }
  execute_sql( sql );
  ...
  strcpy( buff, "" );
  sprintf( insert_sql, "insert into %s values (:i, :x)", tab_name );
  prepare( insert_sql );
  ...
  for( docnum = 1; docnum <= num_docs; docnum++ ) {
	...
	int remainder = k % 2;
      ...
      /* check it doesn't exceed 400000 chars */
      if ( strlen(buff) + strlen(word) < 39980 ) {
