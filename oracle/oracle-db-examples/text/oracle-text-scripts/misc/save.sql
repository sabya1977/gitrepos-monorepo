...
-- TO DO :
-- TAGS hard coded
-- check args - particularly col_type
-- Proper dynamic code
...
create or replace package ctx_cd is
  ...
  procedure do_dynamic_setup (cdname varchar2, username varchar2, tablename varchar2);
...
-- END DEBUGGING CODE
  ...
  for cntr in 1 .. col_cnt loop
    ...
    if (length(sec_names(cntr)) > 0) then
      ...
      insert into trace_source values (proc_buffer);
      dbms_output.put_line(proc_buffer);
      execute immediate (proc_buffer);
    else
      dbms_output.put_line('***Empty section name');
    end if;
  end loop;
