# ADR-005: LEAVE_REQUEST Entity Design for Version 1
...
Accepted ADRs confirm Oracle 26ai as the database platform, Spring JDBC (SimpleJdbcCall) as the database access approach, Java 25 (LTS) as backend language and Spring Boot as the backend platform, and Spring MVC as the API implementation. The physical schema, primary key generation strategy, API contracts, validation approach, authentication, authorization, approval routing, leave balance calculation, cancellation workflow, and detailed audit history remain to be decided.
...
| Attribute                   | Stored or Derived                   | Required?                                | Business Meaning                                                                    | Reasoning                                                                                                                                                                                                                                                                                                                                |
| --------------------------- | ----------------------------------- | ---------------------------------------- | ----------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `LeaveRequestId`            | Stored                              | Yes                                      | Internal surrogate identifier for the leave request.                                | Provides a stable technical identity for relationships from review records and future workflow records. Final generation strategy remains to be decided.                                                                                                                                                                                 |
| `LeaveRequestNumber`        | Stored                              | Yes, if business identifier is confirmed | Business-facing request identifier shown to employees, managers, and support users. | Employees and managers need a readable way to discuss and track requests without exposing only an internal key. Format and generation rules remain to be decided.                                                                                                                                                                        |
| `EmployeeId`                | Stored                              | Yes                                      | Employee who submitted the leave request.                                           | The BRD says every leave request must be associated with an employee. This supports request ownership and manager self-approval checks.                                                                                                                                                                                                  |
| `LeaveTypeId`               | Stored                              | Yes                                      | Requested category of leave.                                                        | The BRD requires employees to identify the leave type being requested. This references maintained leave type master data instead of copying leave type names into each request.                                                                                                                                                          |
| `RequestedStartDate`        | Stored                              | Yes                                      | First date in the requested leave period.                                           | The BRD requires the requested leave period. Start and end dates are the minimum confirmed period information.                                                                                                                                                                                                                           |
| `RequestedEndDate`          | Stored                              | Yes                                      | Last date in the requested leave period.                                            | Required with start date to represent a date range while leaving partial-day and holiday handling undecided.                                                                                                                                                                                                                             |
| `RequestedDayCount`         | Derived for Version 1               | No                                       | Number of leave days represented by the requested period.                           | This depends on unresolved rules for weekends, holidays, partial-day leave, and leave balance calculation. It should be calculated by business logic when needed until those rules are confirmed. If the business later requires preserving the exact evaluated day count at submission time, it may become stored.                      |
| `RequestReason`             | Stored                              | No                                       | Optional employee-provided reason or note for the request.                          | The BRD says the system should capture basic information needed to evaluate a request, but required reasons remain undecided. Keeping this optional supports review context without making it a business rule.                                                                                                                           |
| `CurrentStatusId`           | Stored                              | Yes                                      | Current workflow status of the request.                                             | The BRD requires employees to view current request status and says status changes should be supported as a request moves through review. Detailed status history remains deferred.                                                                                                                                                       |
| `SubmittedDate`             | Stored                              | Yes                                      | Date and time the request was submitted.                                            | Supports ordering, visibility, and auditability of submitted requests. This is a workflow milestone rather than a derived value.                                                                                                                                                                                                         |
| `AssignedManagerEmployeeId` | Stored conditionally                | No                                       | Candidate manager assigned to review the request.                                   | Manager routing is currently represented by `Employee.ManagerEmployeeId` in the ERD. Storing the assigned manager on the request would preserve the routing decision if managers change later, but approval routing rules are not confirmed. For Version 1, this should remain optional or deferred until routing behavior is confirmed. |
| `ReviewedIndicator`         | Derived                             | No                                       | Whether the request has a review outcome.                                           | Can be derived from the existence of a related `LEAVE_REQUEST_REVIEW` record or from a terminal current status. Storing it would duplicate workflow state.                                                                                                                                                                               |
| `ReviewOutcomeCode`         | Derived from `LEAVE_REQUEST_REVIEW` | No                                       | Approved or rejected outcome, when review has occurred.                             | The ERD separates review outcome into `LEAVE_REQUEST_REVIEW`. Duplicating it on `LEAVE_REQUEST` risks inconsistent data unless a later design explicitly denormalizes it.                                                                                                                                                                |
| `PendingManagerEmployeeId`  | Derived or stored conditionally     | No                                       | Manager currently expected to review the request.                                   | Can be derived from the requesting employee's current manager for simple routing. It should only be stored if the business confirms that the assigned reviewer must be frozen at submission time.                                                                                                                                        |
| `CreatedDate`               | Stored                              | Yes                                      | Date and time the row was created.                                                  | Supports basic auditability without requiring a full audit history model.                                                                                                                                                                                                                                                                |
| `CreatedBy`                 | Stored                              | Yes                                      | Actor or process that created the row.                                              | Records who or what created the request record. This is not a foreign key yet because identity storage is undecided.                                                                                                                                                                                                                     |
| `UpdatedDate`               | Stored                              | Yes                                      | Date and time the row was last updated.                                             | Supports troubleshooting and visibility into workflow changes.                                                                                                                                                                                                                                                                           |
| `UpdatedBy`                 | Stored                              | Yes                                      | Actor or process that last updated the row.                                         | Records who or what last changed the request record. This is not a foreign key yet because identity storage is undecided.                                                                                                                                                                                                                |
...
The format is still to be decided. Candidate examples include a simple generated number or a prefixed value such as `LR-2026-000001`, but this ADR does not confirm a format or generation mechanism.
...
| `LeaveRequest` to `LeaveRequestReview`           | One request to zero or one review outcome in the simple Version 1 workflow. | Optional from request to review until review occurs; mandatory from review to request. | Current requirements describe a manager approving or rejecting a request, and the ERD records review outcome separately.                                           |
...
- `ReviewedIndicator`, from review existence or terminal status.
- `ReviewOutcomeCode`, from `LEAVE_REQUEST_REVIEW`.
